...
test.describe('P0_COMPLETE_USER_JOURNEY - 完整用户旅程', () => {
  test('完整用户操作流程', async ({ page }, testInfo) => {
    // 设置测试超时时间
    test.setTimeout(180000); // 3 分钟，完整流程需要更多时间
    ...
    await test.step('步骤 1: 打开首页并验证', async () => {
      ...
      if (!logoFound) {
        ...
        const urlMatches = currentUrl.includes('blacklyte') || currentUrl === base;
      ...
      // 验证首屏内容（避免白屏）
      const mainContent = page.locator('main, .hero, section, .main-content, body > *').first();
    ...
    await test.step('步骤 2: 搜索商品并进入详情页', async () => {
      ...
      const searchButtonVisible = await searchButton.isVisible({ timeout: 3000 }).catch(() => false);
      ...
      if (!searchInputVisible) {
        ...
        // 再次尝试查找
        const retryVisible = await searchInput.isVisible({ timeout: 3000 }).catch(() => false);
        if (!retryVisible) {
          // 如果还是找不到，尝试直接导航到搜索结果页
          // 某些网站支持直接通过 URL 搜索
          await page.goto(`${base}search?q=Athena`, { waitUntil: 'domcontentloaded' });
      ...
      const searchInputFinal = page.locator('input[type="search"], input[name="q"]').first();
      const isInputVisible = await searchInputFinal.isVisible({ timeout: 3000 }).catch(() => false);
      ...
      const searchResults = page.locator('main .product-card, main .search-result-item, main [data-testid*="product"], main .product-item, main a[href*="/products/"]')
        .or(page.locator('.search-results a[href*="/products/"]'))
        .or(page.locator('.predictive-search-results a[href*="/products/"]'))
        .first();
      
      const resultsVisible = await searchResults.isVisible({ timeout: 10000 }).catch(() => false);
      
      if (!resultsVisible) {
        // 如果搜索结果不可见，尝试查找搜索结果区域中的链接（排除导航菜单）
        const athenaLink = page.locator('main a[href*="/products/"]')
          .filter({ hasText: /Athena/i })
          .first();
        
        const linkVisible = await athenaLink.isVisible({ timeout: 5000 }).catch(() => false);
        ...
        if (linkVisible) {
          ...
          // 如果还是找不到，直接导航到商品详情页
          await page.goto(`${base}products/blacklyte-athena-pro-gaming-chair`, { waitUntil: 'domcontentloaded' });
        }
      } else {
        // 点击第一个搜索结果进入详情页
        // 滚动到元素位置，避免被遮挡
      ...
      // 验证商品详情页（使用更宽松的验证）
      const productTitle = page.locator('h1, .product-title, [data-testid="product-title"], .productView-title').first();
      const titleVisible = await productTitle.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    await test.step('步骤 3: 添加商品到购物车', async () => {
      ...
      const addToCartButton = page.getByRole('button', { name: /add to cart|加入购物车|buy now|立即购买/i })
      ...
      const buttonVisible = await addToCartButton.isVisible({ timeout: 10000 }).catch(() => false);
      
      let actualButton = addToCartButton;
      
      if (!buttonVisible) {
        // 如果找不到按钮，尝试查找表单提交按钮
        const formButton = page.locator('form[action*="cart"] button, form[action*="add"] button').first();
        ...
        if (formButtonVisible) {
          ...
          const buyButton = page.locator('button:has-text("Buy"), button:has-text("Add")').first();
          const buyButtonVisible = await buyButton.isVisible({ timeout: 3000 }).catch(() => false);
      ...
      if (isDisabled) {
        // 尝试选择尺寸
        const sizeSelect = page.locator('select[name*="size"], select[id*="size"], [data-testid*="size"]').first();
        const sizeSelectVisible = await sizeSelect.isVisible({ timeout: 3000 }).catch(() => false);
        ...
        // 尝试选择颜色
        const colorOption = page.locator('input[type="radio"][name*="color"], [data-testid*="color"] input, .color-option input').first();
        const colorOptionVisible = await colorOption.isVisible({ timeout: 3000 }).catch(() => false);
        ...
        // 尝试选择第一个可用的选项
        const firstOption = page.locator('input[type="radio"]:not([disabled]), select option:not([disabled])').first();
        ...
        // 重新检查按钮是否已启用
        await page.waitForTimeout(1000);
        const stillDisabled = await actualButton.isDisabled().catch(() => false);
        if (stillDisabled) {
          // 如果仍然禁用，记录警告但继续尝试
      ...
      if (hasRedirected && urlAfterClick.includes('/cart')) {
        // 验证购物车中有商品
        const cartItem = page.locator('.cart-item, [data-testid*="cart-item"], .line-item, tbody tr').first();
      ...
      if (hasRedirected && !urlAfterClick.includes('/cart')) {
        await page.goto(currentUrlBeforeClick, { waitUntil: 'domcontentloaded' }).catch(() => {});
        await page.waitForTimeout(2000);
      }
      ...
      for (const selector of cartSelectors) {
        const element = page.locator(selector).first();
        const text = await element.textContent({ timeout: 3000 }).catch(() => null);
        if (text && text.trim() && /\d/.test(text)) {
      ...
      if (!cartCountAfter) {
        const cartLink = page.locator('a[href*="cart" i]').first();
        const cartLinkText = await cartLink.textContent({ timeout: 3000 }).catch(() => null);
        if (cartLinkText) {
      ...
      if (hadNumberBefore && hasNumberNow && cartCountBefore && cartCountAfter) {
      ...
      // 3. 如果文本内容发生变化（即使没有数字）
      const textChanged = cartCountBefore !== cartCountAfter && cartCountAfter !== null;
      ...
      const inCartButton = page.locator('button:has-text("In Cart"), button:has-text("已在购物车")').first();
      const buttonStateChanged = await inCartButton.isVisible({ timeout: 2000 }).catch(() => false);
        ...
        if (cartLinkVisible) {
          ...
          if (!page.isClosed() && currentUrl) {
            try {
              ...
              await page.goto(cartUrl, { waitUntil: 'domcontentloaded', timeout: 10000 }).catch(() => {});
              if (!page.isClosed()) {
                await page.waitForTimeout(2000).catch(() => {});
              }
            } catch (e) {
        ...
        if (page.isClosed()) {
          ...
          if (isCartPage) {
            ...
            for (const selector of cartItemSelectors) {
              ...
              cartHasItem = await cartItem.isVisible({ timeout: 3000 }).catch(() => false);
              if (cartHasItem) {
                break;
              }
            }
            ...
            if (cartHasItem) {
              ...
              // 购物车为空，返回原页面
              if (!page.isClosed() && currentUrl && !currentUrl.includes('/cart')) {
      ...
      // 至少有一个成功指标
      const addToCartSuccess = cartChanged || hasSuccessMessage || buttonStateChanged || cartHasItem;
    ...
    await test.step('步骤 4: 进入购物车并前往结算', async () => {
      ...
      // 验证购物车页面
      const cartTitle = page.locator('h1, .cart-title, [data-testid="cart-title"]')
        .filter({ hasText: /cart|购物车/i })
        .first();
      await expect(cartTitle).toBeVisible({ timeout: 10000 });

      // 验证购物车中有商品
      const cartItem = page.locator('.cart-item, [data-testid*="cart-item"], .line-item').first();
      await expect(cartItem).toBeVisible({ timeout: 10000 });

      // 点击结算按钮
      const checkoutButton = page.getByRole('button', { name: /checkout|结算|proceed to checkout/i })
      ...
      // 等待跳转到结算页
      await page.waitForURL(/\/checkout|\/cart\/checkout/i, { timeout: 15000 }).catch(() => {});

      // 验证结算页核心元素
      const checkoutForm = page.locator('form, .checkout-form, [data-testid="checkout"]').first();
      const checkoutTitle = page.locator('h1, h2')
        .filter({ hasText: /checkout|结算|shipping|delivery/i })
        .first();

      const hasCheckoutForm = await checkoutForm.isVisible({ timeout: 5000 }).catch(() => false);
      const hasCheckoutTitle = await checkoutTitle.isVisible({ timeout: 5000 }).catch(() => false);
    ...
    await test.step('步骤 5: 处理登录和订阅弹窗', async () => {
      ...
      // 尝试触发登录弹窗（如果存在登录按钮）
      const signInButton = page.getByRole('button', { name: /sign in|login|登录/i })
        .or(page.locator('a:has-text("Sign In")'))
        .first();
      
      const hasSignIn = await signInButton.isVisible({ timeout: 3000 }).catch(() => false);
      if (hasSignIn) {
        await signInButton.click();
        await page.waitForTimeout(1000);

        // 验证登录弹窗出现
        const loginModal = page.locator('[role="dialog"], .modal, .login-modal').first();
        const modalVisible = await loginModal.isVisible({ timeout: 3000 }).catch(() => false);
    ...
    await test.step('步骤 6: 切换地区和币种', async () => {
      // 查找地区选择器
      const regionSelector = page.locator('[data-testid*="region"], [data-testid*="country"], .region-selector')
      ...
      if (hasRegionSelector) {
        ...
        const otherTarget = allTargets.find(t => t.region !== currentTargetConfig.region);
        ...
        if (otherTarget) {
          ...
          const regionName = regionNames[otherTarget.region] || otherTarget.region;
          
          const targetOption = page.getByRole('option', { name: new RegExp(regionName, 'i') })
            .or(page.locator(`a:has-text("${regionName}")`))
            .or(page.locator(`button:has-text("${regionName}")`))
            .or(page.locator(`text=/${otherTarget.region}/i`))
            .first();

          const optionVisible = await targetOption.isVisible({ timeout: 3000 }).catch(() => false);
          if (optionVisible) {
            ...
            // 验证 URL 或页面元素变化
            const currentUrl = page.url();
            const urlChanged = currentUrl.includes(otherTarget.region.toLowerCase()) || 
                             currentUrl.includes(otherTarget.url.replace('https://', '').replace('/', '')) ||
                             currentUrl !== base;
    ...
    await test.step('步骤 7: 访问下载页', async () => {
      ...
      const downloadLink = page.getByRole('link', { name: /download|下载/i })
      ...
      if (hasDownloadLink) {
        ...
        // 验证下载页加载
        const downloadPage = page.locator('h1, .download-title, [data-testid="download"]')
          .filter({ hasText: /download|下载/i })
          .first();
        
        const downloadPageVisible = await downloadPage.isVisible({ timeout: 5000 }).catch(() => false);
        // 下载页可能重定向或直接下载，所以验证更宽松
        expect(downloadPageVisible || page.url().includes('download')).toBeTruthy();
      } else {
        // 如果没有下载链接，跳过此步骤
