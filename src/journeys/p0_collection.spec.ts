  ...
  for (const collection of collections) {
    test(`分类页加载: ${collection}`, async ({ page }) => {
      ...
      const collectionLink = page.getByRole('link', { name: new RegExp(collection, 'i') })
        .or(page.locator(`a:has-text("${collection}")`))
        .first();
      ...
      await page.waitForURL(/\/collections?|\/category|\/shop/i, { timeout: 10000 }).catch(() => {});

      // 验证分类页标题或面包屑
      const pageTitle = page.locator('h1, .page-title, [data-testid="collection-title"]')
        .filter({ hasText: new RegExp(collection, 'i') })
        .first();
      await expect(pageTitle).toBeVisible({ timeout: 10000 });

      // 验证产品列表容器存在
      const productGrid = page.locator('.product-grid, .products, [data-testid="product-list"], .collection-products').first();
      await expect(productGrid).toBeVisible({ timeout: 10000 });

      // 验证至少有一个产品卡片
      const productCard = page.locator('.product-card, .product-item, [data-testid*="product"]').first();
